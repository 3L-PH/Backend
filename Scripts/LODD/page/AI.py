...
def init_close_ear(th_open, both_ear):
    time.sleep(2)
    th_open.join()
    time.sleep(5)
...
def vision(img, TIMER_FLAG, start_closing, COUNTER):
    #1.
    ...
    #9.
    th_open = Thread(target=init_open_ear, args=(both_ear))
    th_open.deamon = True
    th_open.start()
    th_close = Thread(target=init_close_ear, arts=(th_open, both_ear))
    ...
    return TIMER_FLAG, start_closing, COUNTER
